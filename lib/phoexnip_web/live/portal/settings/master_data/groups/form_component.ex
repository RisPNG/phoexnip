defmodule PhoexnipWeb.MasterDataGroupsLive.FormComponent do
  ...
  def update(%{groups: groups} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(CommonService.change(groups))
     end)}
  end

  @impl true
  def handle_event("validate", %{"groups" => groups_params}, socket) do
    changeset = CommonService.change(socket.assigns.groups, groups_params)
  ...
  defp save(socket, :edit, groups_params) do
    case CommonService.update(socket.assigns.groups, groups_params) do
      {:ok, groups} ->
  ...
  defp save(socket, :new, groups_params) do
    case CommonService.create(Phoexnip.Masterdata.Groups, groups_params) do
      {:ok, groups} ->
        Phoexnip.AuditLogService.create_audit_log(
          # Entity type
          "Groups",
