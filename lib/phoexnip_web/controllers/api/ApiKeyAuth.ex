defmodule PhoexnipWeb.Plugs.ApiKeyAuth do
  ...
  This plug:
    ...
    3. If not cached, retrieves the API key record from the database via `Phoexnip.CoreUtils.CommonService` and `Phoexnip.Settings.ApiKey`,
  ...
  defp fetch_and_cache_api_key_data(conn, api_key) do
    case Phoexnip.CoreUtils.CommonService.get_by(Phoexnip.Settings.ApiKey, %{key: api_key}) do
      nil ->
        conn
        |> put_status(:unauthorized)
