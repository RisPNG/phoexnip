defmodule Phoexnip.JobStarter do
  ...
  On startup it waits briefly, then:
    1. Fetches all active scheduler entries from the database via `CommonService.list_where/2`.
  ...
  Handles the `:init_jobs` message by:
    ...
    1. Fetching all active scheduler records via `CommonService.list_where/2`.
  ...
  def handle_info(:init_jobs, state) do
    active_scheduler = CommonService.list_where(Schedulers, status: 1)
